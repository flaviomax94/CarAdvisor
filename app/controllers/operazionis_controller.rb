...
before_action :logged_in_user,
  ...
  def show
	 ...
	 redirect_to(root_path) unless @operazioni.user_id==current_id
  ...
  def create 
      @operazioni = Operazioni.new(operazioni_params)
        respond_to do |format|
	     ...
	     condizione.find do |autoveicolo| 
	     if autoveicolo.kilometri < (@operazioni.km).to_f
			condizione=@autoveicolo.where(user_id: @operazioni.user_id, id: @operazioni.targa)
	      ...
	      format.html { redirect_to '/autoveicolos/'+(@operazioni.targa).to_s}
...
private 
  def calcolo_media_km(id_veicolo)
	...
	condizione.order(data: :desc).each do |operazione|	
  ...
  def operazioni_params
      params.require(:operazionis).permit(:user_id, :km, :oggetto, :officina, :data, :targa, :costo)
  end
